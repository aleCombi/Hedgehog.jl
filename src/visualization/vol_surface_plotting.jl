...
function plot_vol_2d_by_expiry(
    ...
    # Handle :all field - plot mid, bid, ask separately
    plot_all_three = (field == :all)
    if plot_all_three
...
3D scatter/surface plot over (Strike, Time to Maturity). Suitable for irregular grids.
...
- `show_all`: If true and field=:all, creates 3 separate subplots for mid/bid/ask
...
function plot_vol_surface_3d(
    ...
    # Handle :all - either combine in one plot or separate subplots
    if field == :all
        if show_all
